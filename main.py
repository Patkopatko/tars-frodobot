...
async def control(request: Request):
    ...
    try:
        ...
        logger.warning("browser_service failed (%s), falling back to RTM REST", str(e))
        try:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, lambda: RtmClient(auth_response_data).send_message_to_bot(command))
            return {"message": "Command sent via RTM fallback"}
        except Exception as e2:
            logger.error("RTM fallback also failed: %s", str(e2))
            raise HTTPException(status_code=500, detail="Failed to send control command") from e2
